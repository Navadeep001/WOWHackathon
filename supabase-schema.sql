-- ─────────────────────────────────────────────────────────
-- LegoLink — Supabase SQL Schema (fixed)
...
-- user_hackathons
create policy "Users can read own hackathon registrations"
...
-- team_invites
create policy "Users can read their own invites or invites for teams they own"
...
create policy "Users can send join requests"
  on team_join_requests for insert with check (auth.uid() = requesting_user_id);
...
-- chat_messages
-- FIX: use = any() instead of = for uuid[] comparison
...
-- ─────────────────────────────────────────────────────────
-- Functions for accepting invites and join requests
-- (handles current_member_ids update atomically)
-- ─────────────────────────────────────────────────────────
